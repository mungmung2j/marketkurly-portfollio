                                ...
                                {state.상품.map((item, idx)=>{
                                    return(
                                        <li className={`slide slide${idx+1}`} key={item.제품코드}>
                                            <div className="gap" onClick={(e)=>onClickViewProduct(e, item, './img/intro/section8/')}>
                                                <div className="img-box"><img src={`./img/intro/section8/${item.제품이미지}`} alt="" /></div>
                                                <div className="txt-box">
                                                    <a href="!#"><img src="./img/intro/section8/icon_cart_black.svg" alt="" />담기</a>
                                                    <h3>{item.제품명}</h3>
                                                    <h4>{item.정가}원</h4>
                                                    <h5><strong>{(item.할인율)*100}%</strong> <em>{item.정가*(1-item.할인율)}원</em></h5>
                                                    <h6><img src="./img/intro/section8/icon_mal.svg" alt="" />{item.리뷰}</h6>
                                                </div>
                                            </div>
                                        </li>
                                    )
